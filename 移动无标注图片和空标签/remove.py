...
# 设置原始标签路径为 Annos
origin_ann_dir = r'E:\code\py脚本\dataset\Annotations\\'
origin_pic_dir = r'E:\code\py脚本\dataset\JPEGImages\\'
# 设置新标签路径 Annotations
new_ann_dir = r'E:\code\py脚本\dataset\new\anno\\'
new_pic_dir = r'E:\code\py脚本\dataset\new\img\\'
...
for dirpaths, dirnames, filenames in os.walk(origin_ann_dir):
    for filename in filenames:
        ...
        if os.path.isfile(r'%s%s' % (origin_ann_dir, filename)):
            # 如果是，获取绝对路径（重复代码）
            origin_ann_path = os.path.join(
                r'%s%s' % (origin_ann_dir, filename))
