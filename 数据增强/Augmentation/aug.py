...
#
#     return image
def roi_resize(image, shape, area_max=2000, area_min=1000 ):
    # 改变图片大小
    height, width, channels = image.shape
    ori_height, ori_width, _ = shape
    short = min(ori_height, ori_width)
    arg = int(short/5)
    # while (height * width) > area_max:
    #     image = cv2.resize(image, (int(width * 0.9), int(height * 0.9)))
    #     height, width, channels = image.shape
    ...
    #     height, width = int(height * 1.1), int(width * 1.1)
    if height > width:
        image = cv2.resize(image, (arg, int(height/(width/arg))))   # 注意，目标size不能太大，否则图片会不够大小贴下目标
    else:
        image = cv2.resize(image, (int(width/(height/arg)), arg))
...
def copysmallobjects(image_dir, label_dir, save_base_dir, small_img_dir,
    ...
    for small_img_dirs in small_img_dir:
        ...
        # from 3000 to 1500
        roi = roi_resize(image_bbox, image.shape, area_max=1000, area_min=200)  # 对roi图像做缩放
    ...
    yolo_txt_dir = join(
        save_dir,
        basename(image_dir.replace('.jpg', '.txt')))
    cv2.imwrite(
        join(save_dir,
             basename(image_dir).replace('.jpg', '.jpg')),
        image)
