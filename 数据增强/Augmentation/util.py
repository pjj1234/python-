...
def random_add_patches(bbox_img, rescale_boxes, shape, paste_number, iou_thresh, smallimg):
    temp = []
    ...
    new_bboxes = []
    # 需要贴图的标签，每次只能贴一个标签目标
    # label = smallimg.replace('.jpg', '.txt')
    cl = open(smallimg.replace('.jpg', '.txt'), "r").readlines()[0]
    # cl = 0
